...
ENVSIM_CONFIG_PATH = "/root/phase3-hurdle/gr-envsim/apps/envsim.ini"
...
def main():
    ...
    parser.add_argument('--clean-competitor-containers', action="store_true", default=False,
                        help=("When specified, this flag will make the run script remove the competitor containers" +
                              " at the end of a run"))
    ...
    parser.add_argument('--enable-debug-output', action="store_true", default=False,
                        help=("When specified, this flag will run the envsim with a ZMQ push socket that outputs" +
                              " the samples sent to competitor containers"))
    ...
    # used only for internal debug
        ...
        raise ValueError("Unknown bot mode {} specified".format(args["bot_mode"]))
    ...
    # build list of expected competitor container names
    all_containers = NUM_BOT_CONTAINERS+NUM_COMPETITOR_CONTAINERS
    comp_container_names = [COMPETITOR_NAME_BASE+"{}".format(i+1) for i in range(NUM_BOT_CONTAINERS, all_containers)]
    ...
    # update list of all containers currently loaded
    my_containers = lxd_client.containers.all()
    ...
    packets_per_container = args["packet_rate"]*(args["duration"]-bootup_slop_time)
    # each bot container sends packets to all other containers but itself
    num_packets = math.floor(NUM_BOT_CONTAINERS*(NUM_BOT_CONTAINERS-1)*packets_per_container)
